import { createServerClient } from "@/lib/supabase/serverClient";
import Link from "next/link";
import StockClient from "./stockClient";

interface StockRow {
  dye_item_id: string;
  stock_qty: number;
  issued_qty: number;
  dye_items: {
    id: string;
    name: string;
    type: string | null;
    code: string | null;
    uom: string;
  };
}

export default async function DyesStockPage() {
  const supabase = await createServerClient();

  const [stockResult, issuedResult] = await Promise.all([
    supabase
      .from("dye_stock")
      .select(
        `
        dye_item_id,
        stock_qty,
        dye_items:dye_item_id (
          id,
          name,
          type,
          code,
          uom
        )
      `
      ),
    supabase
      .from("dye_transactions")
      .select("dye_item_id, quantity")
      .eq("transaction_type", "ISSUE"),
  ]);

  const { data: stockData, error } = stockResult;
  const { data: issuedData } = issuedResult;

  // Aggregate issued quantities by dye_item_id
  const issuedByItem: Record<string, number> = {};
  (issuedData || []).forEach((row: { dye_item_id: string; quantity: number }) => {
    const id = row.dye_item_id;
    const qty = Number(row.quantity || 0);
    issuedByItem[id] = (issuedByItem[id] || 0) + qty;
  });

  const processed: StockRow[] =
    (stockData as any[])?.map((row) => ({
      ...row,
      stock_qty: Number(row.stock_qty || 0),
      issued_qty: issuedByItem[row.dye_item_id] ?? 0,
      dye_items: Array.isArray(row.dye_items) ? row.dye_items[0] : row.dye_items,
    })) || [];

  // Sort by name
  processed.sort((a, b) => (a.dye_items?.name || "").localeCompare(b.dye_items?.name || ""));

  return (
    <div className="grid gap-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-semibold text-slate-900">
            Dyes &amp; Chemicals Stock
          </h1>
          <p className="mt-1 text-slate-600">
            In-store and issued balances for dyes &amp; chemicals.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Link
            href="/toolbox/dyes/stocktake"
            className="text-sm font-semibold text-teal-700 hover:text-teal-800 transition"
          >
            Month-end Stocktake
          </Link>
          <Link
            href="/toolbox/dyes"
            className="text-sm font-semibold text-slate-700 hover:text-slate-900 transition"
          >
            ← Back to Dyes &amp; Chemicals
          </Link>
        </div>
      </div>

      <StockClient initialStock={processed} />
    </div>
  );
}

